...
define :git_clone, {
  ...
  bash "update git clone of #{git_clone_params[:repository]} to #{git_clone_params[:name]}" do
    user git_clone_params[:user]
    code "cd #{git_clone_params[:name]} && git reset --hard -q && git clean #{clean_options} && (echo -n #{git_clone_params[:reference]} | grep -q '^[0-9a-f]\\{40\\}$' && git remote update || (git checkout #{git_clone_params[:reference]} && git pull)) && git checkout #{git_clone_params[:reference]}"
