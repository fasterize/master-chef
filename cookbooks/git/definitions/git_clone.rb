...
define :git_clone, {
  ...
  bash "update git clone of #{git_clone_params[:repository]} to #{git_clone_params[:name]}" do
    user git_clone_params[:user]
    code "cd #{git_clone_params[:name]} && git reset --hard -q && git clean #{clean_options} && ((git branch --no-color | tr -d '*' | grep -q '^[ ]*#{git_clone_params[:reference]}$' && git checkout #{git_clone_params[:reference]} && git pull) || git remote update) && git checkout #{git_clone_params[:reference]}"
